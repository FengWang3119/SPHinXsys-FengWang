...
//=================================================================================================//
    ...
    void P_refinement::update(size_t index_i, Real dt)
    {
        ...
        if (is_near_wall_P1_[index_i] == 1)
        {
            ...
            else if (type_sublayer_solver == 3) //** If go ConstantPG Path *
            {
                if (1) //** Whether probe iteration *
                {
                    ...
                    while (residue > 1.0e-3)
                    {
                        ...
                        U_nodeUM = 0.0;
                        velocity_gradient_nodeO = 0.0;
                        node_value_vel_[index_i] = solve_1D_sublayer_constantPG(nu, u_outer, k_outer, omega_outer, std::abs(dudn_outer),
                            nut_outer, distance_to_wall, friction_vel_magnitude_outer, std::abs(flow_rate_local), dkdn_outer, dwdn_outer, U_nodeO, U_nodeUM, velocity_gradient_nodeO);
            ...
            //** Check results *
            if (!std::isfinite(sublayer_result.sublayer_utau))
            {
    ...
    //=================================================================================================//
    Vec6d P_refinement::solve_1D_sublayer_constantPG2(double kinematic_viscosity, double u_p_outer, double k_p_outer,
        ...
        double Q_target, double k_grad_p_outer, double w_grad_p_outer, double& vel_nodeO, double& vel_nodeUM, double& vel_grad_nodeO_)
    {
        ...
        //------------------------------------------------↓ Calculate nodeO and nodeUM, the vel_grad_from_SPH is used here only as initial value↓------------------------------------------------
        //** These values need tobe updated in each iteration *
        ...
        double Q_target, double k_grad_p_outer, double w_grad_p_outer, double& vel_nodeO, double& vel_nodeUM)
    {
        ...
        while (differ > convergence_criteria_outer)
        {
            ...
            //-------------------------------------↓ IF fully implicity, dpdx can be diffrent ↓-------------------------------------
        ...
        double Q_target, double k_grad_p_outer, double w_grad_p_outer, double& vel_nodeO, double& vel_nodeUM)
    {
        ...
        while (differ > convergence_criteria_outer)
        {
            ...
            //-------------------------------------↓ IF inner node explicit, P adjacent node implicit, constant dpdx assumption ↓-------------------------------------
            ////** explicity part *
            ...
            //-------------------------------------↑ IF fully implicity, P adjacent node implicit, dpdx can be diffrent ↑-------------------------------------
